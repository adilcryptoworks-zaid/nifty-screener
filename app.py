...
def analyze_ticker(ticker):
    out = {"ticker": ticker}
    try:
        # Fetch 3-minute intraday for last 3 days (yfinance supports '3m' interval)
        ...
        # rising structure: last 3 highs increasing and lows increasing
        ...
        # Long-term indicators (1h or daily)
        long_ema50 = np.nan
        long_ema200 = np.nan
        ...
        # Delivery trend: use daily deliverable percent if available
        delivery_pct = None
        if "Deliverable" in df_daily.columns:
            # unlikely; keep placeholder
            delivery_pct = None
        # Compose result
        out.update({
...
if refresh_button:
    ...
    df_display = df[["ticker","last_price","rsi_14","ema20","ema50","momentum_bullish","trend_strong","rising_structure","reversal_unexpected","long_rising_structure","long_trend_strong"]]
    st.subheader("Results")
    st.dataframe(df_display.style.applymap(lambda v: 'background-color: #b6f0c2' if v==True else ('background-color: #ffd6d6' if v==False else ''), subset=["momentum_bullish","trend_strong","rising_structure","reversal_unexpected","long_rising_structure","long_trend_strong"]), height=600)
